...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <sch:pattern>
    ...
    <sch:rule context="//f:MedicationStatement">
      ...
      <sch:assert test="not(exists(*[starts-with(local-name(.), 'reasonForUseReference')]) and (f:notTaken/@value='Y' or f:notTaken/@value='UNK'))">mst-3: Reason for use reference is only permitted if notTaken is No</sch:assert>
      ...
      <sch:assert test="not(exists(*[starts-with(local-name(.), 'reasonForUseCode')]) and (f:notTaken/@value='Y' or f:notTaken/@value='UNK'))">mst-2: Reason for use code is only permitted if notTaken is No</sch:assert>
    </sch:rule>
